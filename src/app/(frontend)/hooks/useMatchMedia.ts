...
const useMatchMedia = <
  ...
  TDefaultValue extends TConfig['defaultValue'] | never = TConfig['defaultValue'] extends undefined
    ? undefined
...
>(
  ...
  const getExistingMatch = useCallback(() => {
    ...
    return config && 'defaultValue' in config ? (config.defaultValue as TDefaultValue) : false
  }, [config, query])
  ...
  const createEventHandler = (query: string) => (event: MediaQueryListEvent) => {
    ...
    if (matchedQuery) {
      matchedQuery.existingListeners.forEach((listener: (arg0: MediaQueryListEvent) => unknown) =>
        listener(event),
      )
    }
  }
  ...
  const removeListener = useCallback((query: string, listener: EventHandler) => {
    const matchedQuery = QUERIES.get(query)
  ...
  const [matches, setMatches] = useState<boolean | TDefaultValue>(getExistingMatch)

  const useEffect = config?.layoutEffect ? useIsomorphicLayoutEffect : React.useEffect
